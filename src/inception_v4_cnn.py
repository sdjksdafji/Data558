...
def get_pretrained_model():
    return inception_v4(include_top=False, weights='imagenet', pooling="avg")
...
def save_bottlebeck_features(
    ...
    generator = get_train_generator(width=inception_v4_width, height=inception_v4_height, batch_size=3200)
    i = 0
    ...
    generator = get_test_generator(width=inception_v4_width, height=inception_v4_height, batch_size=3200)
    i = 0
    for batch_x, batch_y in generator:
        batch_x = applications.inception_v3.preprocess_input(batch_x)
        bottleneck_batch_x = pretrained_model.predict(batch_x)
...
def fine_tuning_cnn():
    ...
    model.fit_generator(
        get_train_generator(width=inception_v4_width, height=inception_v4_height, preprocessing_function=applications.inception_v3.preprocess_input),
        steps_per_epoch=100,
        epochs=500,
        validation_data=get_test_generator(width=inception_v4_width, height=inception_v4_height, preprocessing_function=applications.inception_v3.preprocess_input),
        validation_steps=25,
        workers=7,
        callbacks=[earlystopper])
